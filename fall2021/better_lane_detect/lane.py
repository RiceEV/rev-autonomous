...
class Lane:
  ...
  def __init__(self, orig_frame, roi):
    ...
    '''
    self.roi_points = np.float32([ #960 x 540 is the video, 600x338 is the image
      (284*(960/600),178*(540/338)), # Top-left corner
      (10*(960/600), 310*(540/338)), # Bottom-left corner
      (510*(960/600),310*(540/338)), # Bottom-right corner
      (320*(960/600),178*(540/338)) # Top-right corner
    ])
    '''
    self.roi_points = roi
...
def run(original_frame, frame_number):

  # Load a frame (or image)
  #original_frame = cv2.imread(filename)

  """ roi_pts = np.float32([ #960 x 540 is the video, 600x338 is the image
    (284*(960/600) + 50*math.cos(frame_number/30 * 2 * 3.14), 178*(540/338)), # Top-left corner
    (10*(960/600), 310*(540/338)), # Bottom-left corner
    (510*(960/600),310*(540/338)), # Bottom-right corner
    (320*(960/600) + 50*math.sin(frame_number/30 * 2 * 3.14), 178*(540/338)) # Top-right corner
  ]) """
  roi_pts = np.float32([(422, 285),(16,496),(816,496),(549,284)])

  # Create a Lane object
  lane_obj = Lane(orig_frame=original_frame, roi=roi_pts)
...
def training_from_video(filepath):
  video_object = cv2.VideoCapture(filepath)
  frame_freq = 30
  total_frames = 0
  while True:
    ret, frame =video_object.read()
    total_frames += 1
    if total_frames%frame_freq != 0 | ret is False:
      continue
    run(frame, total_frames)
    if cv2.waitKey(10) & 0xFF == ord('q') :
        # break out of the while loop
        break
...
def main():
  ...
  # frame = 'C:\\Users\\phuso\\OneDrive - Rice University\\Notebooks\\2021 Fall\\REV\\frames'
  ...
  training_from_video('drivingvideo.mp4')
  #training_from_camera()
